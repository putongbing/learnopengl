...
int main()
{
	...
	unsigned int gsId = shader.createShader(CShader::GEOMETRY);
	comRet = shader.compileShaderByFile(gsId, "./shader//model.gs");
	if (comRet != 1) {
		char err[1024];
		shader.getCompileOrLinkInfo(gsId, CShader::COMPILE, err);
		return 0;
	}
	...
	unsigned int lstId[] = { vectexId , gsId, fragId };
	comRet = shader.linkShader(3, lstId);
	if (comRet != 1)
		return 0;
	glm::mat4 model(1.0);
	glm::mat4 view(1.0);
