...
char* CShader::getCompileOrLinkInfo(GLuint id, SHADER_STAUS status) {
	char info[1024];
	GLsizei length;
	if (status == COMPILE)
		glGetShaderInfoLog(id, 1024, &length, info);
	else
		glGetProgramInfoLog(id, 1024, &length, info);
	return info;
}
...
bool CShader::setVec2(std::string name, std::vector<float> value) {
	if (value.size() < 2) {
		std::cout << "Value is little!" << std::endl;
		return -1;
	}
	GLint location = glGetUniformLocation(m_programID, name.c_str());
	if (location == -1) {
...
bool CShader::setVec3(std::string name, std::vector<float> value){
	if (value.size() < 3) {
		std::cout << "Value is little!" << std::endl;
		return -1;
	}
...
bool CShader::setVec4(std::string name, std::vector<float> value) {
	if (value.size() < 4) {
		std::cout << "Value is little!" << std::endl;
		return -1;
	}
...
bool CShader::setMatrix4x4(std::string name, std::vector<float> value) {
	if (value.size() < 16) {
		std::cout << "Value is little!" << std::endl;
		return -1;
	}
	...
	glUniformMatrix4fv(location, value.size(), false, arrValue);
	return true;
}
