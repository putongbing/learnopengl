...
public:
	...
	*/
	...
	param-status:编译或链接
	param-error(in):需要获取的错误信息
	*/
	void getCompileOrLinkInfo(GLuint id, SHADER_STAUS status, char* error);
	/*
	linkShader:将着色器进行链接
	...
	*/
	...
	bool setVec2(std::string name, glm::vec2 value);
	bool setVec3(std::string name, glm::vec3 value);
	bool setVec4(std::string name, glm::vec4 value);
	bool setMatrix4x4(std::string name, glm::mat4 value);
	void use();
